...
% BemGEF_list: a list of various number of Bem1-GEF used to generate the
% bifurcation diagram.
% seeds: initialize a random number generator.
% standard_num_K: total number of Ripley's K values from each simulation. 
...
% '../K_values/2DCombinedPolarity' contains K values of the 2D combined
% polarity circuit model simulations.
% '../K_values/2DCorePolarity' contains K values of the 2D core polariy
% circuit model simulations.

% Outputs:
% unpolarized_regime: index of the unpolarized regime in BemGEF_list and
% simulated_K_diff.
% transient_polarity_regime: index of the transient polarity regime in 
% BemGEF_list and simulated_K_diff.
% polarized_regime: index of the polarized regime in BemGEF_list and
% simulated_K_diff.
% simulated_K_diff: Difference between K>=1.5 and K<1.5 estimated from a 
% sigmoidal function of BemGEF_list.
% K_diff: Differences between K>=1.5 and K<1.5 calculated from coordinates 
% of Cdc42-GTP of the particle-based simulations. It is used as
% experimental data points to fit the sigmoidal function and estimate the 
% relation between difference in K and number of Bem1-GEF.
...
% load Ripley's K values, number of Cdc42 and number of Bem1-GEF in each 
% particle-based simulation. Each ".mat" file was titled with a certain  
...
for k = 1:numel(n_BemGEF)
    for j = 1:seeds
        ...
        % an initial condition of unpolarizedly distributed Cdc42
        % K2 is a list of K values collected from simulations with
        % an initial condition of a pre-polarized Cdc42 cluster.
...
% -1 * 201(number of K in each simulation) * 10(number of realizations for 
% a particular number of Bem1-GEF) * 2(simulations are performed with 2 
% different initial conditions) = -4020.
K_diff = [-4020,K_diff]; 

% Use the K_diff value closest to zero as the guess value of 
% the parameter c for sigmoidal curve fitting. 
...
% Generate simulated difference in frequency of K
...
transient_polarity_regime = find(simulated_K_diff>=lower_threshold & simulated_K_diff<=upper_threshold);
